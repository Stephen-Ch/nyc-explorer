...
### AMBIGUITY CARD MACRO
```text
Ambiguity Card — <STORY-ID>
Questions:
- Q1: <question>
- Q2: <question>
...
|-------------|-----------------|
| Docs-only | None (note "docs-only" in response) |
| Runtime/code | Full test suite + typecheck |

### Logging Policy
*   After **GREEN**: Update `docs/code-review.md` (one line) + `docs/project/ai-project-status.md` if architecture changed.
*   After **RED**: Log in `docs/code-review.md` only.

---

## 7. Pause Rule
After a backlog item is GREEN and logged, **STOP**. Await explicit user cue ("NEXT" or "PROCEED").

...
| Role | Responsibility |
|------|----------------|
| **Copilot** | Creates the x-branch, initiates the agent, runs the spike, runs gates, drafts the spike report, opens/updates the PR. |
| **ChatGPT** | Writes the spike prompt, reviews the report, makes the adoption decision (A/B/C/D). |
| **Stephen** | Keeps working on main in parallel. Reviews PR as a review envelope. Approves or rejects promotion of learning back to main. |
...
| Named test or fixture | Cherry-picked or rewritten on main in a real story |
...
| **B — Adopt learning + named test/fixture** | Learning captured, plus a specific test or fixture is cherry-picked or rewritten on main. |
...
Every spike report must pass a hostile scrutiny gate before an adoption decision is made. The gate asks:
...
If any answer is NO, the report must be revised before adoption.
...
- Promoted tests land on main via a normal story commit, not via merge from the spike branch.
